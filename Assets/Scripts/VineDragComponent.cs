...
public class VineDragComponent : MonoBehaviour
{
	...
	private static SeedizenComponent draggingSeedizen;
	private static Vector3 startDrag;
	...
	public static void StartVineDrag (VineComponent vc, SeedizenComponent dragSeedizen)
	{
		...
		foreach (var seedizen in vine.seedizens)
		{
			...
			seedizen.dragStartPosition = seedizen.transform.position;
			...
			if (seedizen.Equals (dragSeedizen))
			{
				startDrag = seedizen.transform.position;
				draggingSeedizen = dragSeedizen;
			}
